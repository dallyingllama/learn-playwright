...
Rollback to Asciidoctor local docs flow:

* Use the `Local Docs Preview` commands in this file (`docs:build` and `docs:preview`).

CI docs rollback path:

* In `.github/workflows/playwright.yml`, set `DOCS_PIPELINE: asciidoctor`.
* The workflow keeps both docs paths:
  ** Antora path: `docs:build:antora` + `docs:verify:antora`
  ** fallback path: Asciidoctor build block
* Switch `DOCS_PIPELINE` back to `antora` to return to the Antora publish path.
