...
* run unit tests first as a fail-fast gate
* run the `@sanity` subset if unit tests pass
...
* build and publish Antora documentation to GitHub Pages
...
* the published `index.html` checks `version.json` and auto-refreshes once when browser cache serves an older report list
