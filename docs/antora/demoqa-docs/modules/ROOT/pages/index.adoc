= Playwright Test Architecture Lab
:icons: font
...
This repository is a practical Playwright + TypeScript test architecture project built around `demoqa.com`.
...
* one `@sanity` baseline test in each spec file
...
* generated local docs via Antora
...
[source,bash]
----
corepack pnpm run test:ui:spec:dev tests/login.spec.ts
----
...
* link:developer-how-to.html[Developer How-To]
...
The CI workflow uses `@sanity` as a small gate stage before the full suite and supports staged execution.
